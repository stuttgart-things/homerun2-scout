...
func TestTokenAuthMiddleware_ValidToken(t *testing.T) {
	handler := TokenAuthMiddleware("test-token", func(w http.ResponseWriter, r *http.Request) {
...
func TestTokenAuthMiddleware_EmptyToken(t *testing.T) {
	...
	if rr.Code != http.StatusOK {
		t.Errorf("status = %d, want %d (empty token should skip auth)", rr.Code, http.StatusOK)
	}
}
